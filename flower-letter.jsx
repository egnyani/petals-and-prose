...
// ── API config: when deployed to Vercel, use "" (same origin) ──
const API_BASE = "";
...
// ── Bouquet-style stem: shorter, curves toward bouquet center ───────────────
// stemCurve: organic bend; stemBend: pulls stem toward bouquet center
function StemmedFlower({ flower, size = 1, stemCurve = 0, stemBend = 0 }) {
  const headSz = Math.round(48 * size);   // fuller flower heads for bouquet
  const stemH  = Math.round(88 * size);   // longer stems
  const svgW   = Math.round(56 * size);
  const cx     = svgW / 2;
  const sw     = Math.max(2, 3.5 * size);

  // Curve toward bouquet center + organic wobble
  const bend = stemBend * 18 * size;
  const cp1x = cx + stemCurve * 12 * size + bend * 0.4;
  const cp1y = stemH * 0.3;
  const cp2x = cx - stemCurve * 10 * size + bend * 0.8;
  const cp2y = stemH * 0.75;
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", userSelect: "none", lineHeight: 1 }}>
      ...
      <svg width={svgW} height={stemH} style={{ display: "block", overflow: "visible" }}>
        <path
          ...
          stroke="#2d6b2d" strokeWidth={sw} fill="none" strokeLinecap="round"
        />
        {flower.hasLeaf && (
          <ellipse
            cx={lx} cy={ly}
            rx={Math.round(10 * size)} ry={Math.round(5 * size)}
            fill="#3d8b3d"
...
export default function FlowerLetter() {
  ...
  const addFlower = f => {
    ...
    if (!r) return;
    uid.current++;
    // Bouquet center: stems converge at bottom center
    const centerX = r.width / 2;
    const centerY = r.height - 75;
    // Place flowers in a dome arc above the bouquet center
    const angle = (placed.length / Math.max(1, placed.length + 1)) * 2.2 - 1.1 + (Math.random() - 0.5) * 0.8;
    const radius = 45 + placed.length * 4 + Math.random() * 25;
    const x = centerX + Math.cos(angle) * radius;
    const y = centerY - 90 - Math.abs(angle) * 35 - Math.random() * 30;
    const next = [...placed, {
      uid: uid.current, ...f,
      x: Math.max(40, Math.min(r.width - 40, x)),
      y: Math.max(25, Math.min(r.height - 130, y)),
      rot:       (Math.random() - 0.5) * 18,
      scale:     0.88 + Math.random() * 0.24,
      stemCurve: (Math.random() - 0.5) * 2.0,
      stemBend:  (centerX - x) / (r.width * 0.4),  // bend stem toward bouquet center
    }];
  ...
  const MIN_SCALE = 0.5, MAX_SCALE = 1.6, SCALE_STEP = 0.08;
  const adjustFlowerSize = (uid, delta) => {
    saveHist(placed);
    setPlaced(p => p.map(f => f.uid === uid ? { ...f, scale: Math.max(MIN_SCALE, Math.min(MAX_SCALE, (f.scale ?? 1) + delta)) } : f));
  };
  ...
  const onMove = useCallback(e => {
    ...
    const r  = canvasRef.current.getBoundingClientRect();
    const centerX = r.width / 2;
    const newX = Math.max(20, Math.min(r.width-20,  cx-r.left-dragOffset.x));
    const newY = Math.max(10, Math.min(r.height-10, cy-r.top -dragOffset.y));
    setPlaced(p => p.map(f => f.uid === dragging
      ? { ...f,
          x: newX, y: newY,
          stemBend: (centerX - newX) / (r.width * 0.4) }
      : f));
  }, [dragging, dragOffset]);
  ...
  const centerInCanvas = (t0, t1, r) => ({
    x: ((t0.clientX + t1.clientX) / 2) - r.left,
    y: ((t0.clientY + t1.clientY) / 2) - r.top,
  });

  const handleTouchStart = useCallback((e, flowerUid) => {
    if (!e.touches?.length || !canvasRef.current) return;
    const f = placed.find(p => p.uid === flowerUid);
    if (!f) return;
    if (e.touches.length === 1) {
      touchStateRef.current = null;
      e.preventDefault();
      startDrag(e, flowerUid);  // single finger = move (same as mouse drag)
      ...
      const c = centerInCanvas(e.touches[0], e.touches[1], r);
      touchStateRef.current = {
        flowerUid, mode: "pinch",
        lastDist: d, lastScale: f.scale ?? 1,
        lastCenterX: c.x, lastCenterY: c.y,
        lastFlowerX: f.x, lastFlowerY: f.y,
      };
    }
  }, [placed]);
  ...
  const handleTouchMove = useCallback((e) => {
    ...
    if (!ts || !e.touches?.length || !canvasRef.current) return;
    e.preventDefault();
    const r = canvasRef.current.getBoundingClientRect();
    const centerX = r.width / 2;
    if (e.touches.length >= 2 && ts.mode === "pinch") {
      ...
      const newScale = Math.max(MIN_SCALE, Math.min(MAX_SCALE, ts.lastScale * scaleFactor));
      const deltaX = c.x - ts.lastCenterX;
      const deltaY = c.y - ts.lastCenterY;
      const newX = Math.max(20, Math.min(r.width - 20, ts.lastFlowerX + deltaX));
      const newY = Math.max(10, Math.min(r.height - 10, ts.lastFlowerY + deltaY));
      ts.lastDist = d; ts.lastScale = newScale;
      ts.lastCenterX = c.x; ts.lastCenterY = c.y;
      ts.lastFlowerX = newX; ts.lastFlowerY = newY;
      saveHist(placed);
      setPlaced(p => p.map(f => f.uid === ts.flowerUid ? { ...f, scale: newScale, x: newX, y: newY, stemBend: (centerX - newX) / (r.width * 0.4) } : f));
    }
  }, [placed]);

  const handleTouchEnd = useCallback((e) => {
    if (!e.touches?.length) touchStateRef.current = null;
  }, []);
  ...
  const letterText = `${subject ? subject+"\n\n" : ""}${letter}\n\n— ${sender}`;
  const copyLetter = () => {
    navigator.clipboard.writeText(letterText).then(() => setToast("Copied to clipboard ✓"));
  };

  const canSend3 = recipient.name && recipient.email && !emailErr && sender;

  const sendLetter = async () => {
    if (!canSend3 || sending) return;
    setSendError(null);
    setSending(true);
    try {
      const res = await fetch(`${API_BASE}/api/send-letter`, {
        ...
        body: JSON.stringify({
          ...
          from_name: sender,
          subject: subject || `A letter from ${sender}`,
          message: letterText,
        }),
      });
      ...
      setSent(true);
      setToast("Letter sent! ✓");
    } catch (err) {
      setSendError(err.message || "Failed to send. Deploy to Vercel and add RESEND_API_KEY.");
      setToast("Could not send. See instructions below.");
    } finally {
      setSending(false);
    }
  };
  ...
  const reset = () => {
    ...
    setRecipient({name:"",email:""}); setSender(""); setSent(false); setSendError(null); setEmailErr("");
  };
      ...
      <div style={{ maxWidth:820, margin:"0 auto", padding:"32px 28px" }}>
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ color:P.driftwood, fontSize:12, margin:"0 0 16px" }}>
              Click to add · drag to move · pinch to resize · click a flower to select
            </p>
            ...
            <div ref={canvasRef}
              ...
              onTouchMove={e => { if (touchStateRef.current) handleTouchMove(e); else if (dragging) { e.preventDefault(); onMove(e); } }}
              onTouchEnd={e => { handleTouchEnd(e); endDrag(); }}
              onMouseDown={e => { if (e.target === canvasRef.current) setActiveFlower(null); }}
              style={{
              ...
              {placed.map(f => {
                ...
                return (
                  <div key={f.uid}
                    ...
                    onTouchStart={e => handleTouchStart(e, f.uid)}
                    onClick={e => { if (!isDragged) setActiveFlower(prev => prev === f.uid ? null : f.uid); }}
                    style={{
                    ...
                    <StemmedFlower flower={f} size={1} stemCurve={f.stemCurve} stemBend={f.stemBend ?? 0} />
                  </div>
                );
              })}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12, flexWrap:"wrap", gap:8 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                ...
                <GhostBtn onClick={() => { if (activeFlower) { removeFlower(activeFlower); setActiveFlower(null); } }} disabled={!activeFlower} hoverColor={P.coral}>Delete</GhostBtn>
                <GhostBtn onClick={clear} disabled={!placed.length} hoverColor={P.coral}>✕ Clear</GhostBtn>
                {activeFlower && (
                  <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                    <span style={{ color:P.driftwood, fontSize:11, fontFamily:BODY }}>Size</span>
                    <button onClick={() => adjustFlowerSize(activeFlower, -SCALE_STEP)} disabled={!activeFlower}
                      style={{ width:28, height:28, background:P.sage, border:"none", borderRadius:8, color:P.bark, fontSize:16, fontWeight:700, cursor:"pointer", lineHeight:1 }}>−</button>
                    <button onClick={() => adjustFlowerSize(activeFlower, SCALE_STEP)} disabled={!activeFlower}
                      style={{ width:28, height:28, background:P.sage, border:"none", borderRadius:8, color:P.bark, fontSize:16, fontWeight:700, cursor:"pointer", lineHeight:1 }}>+</button>
                  </div>
                )}
        ...
        {step === 3 && !sent && (
          <div>
              ...
              <div style={{ width:82, height:112, background:"rgba(255,255,255,0.35)",
                ...
                {placed.slice(0,7).map((f,i) => (
                  <div key={i} style={{
                    ...
                    <StemmedFlower flower={f} size={0.85} stemCurve={f.stemCurve||0} stemBend={f.stemBend??0} />
                  </div>
                ))}
            ...
            <div style={{ display:"flex", gap:10, justifyContent:"space-between", marginTop:16, flexWrap:"wrap" }}>
              <GhostBtn onClick={() => setStep(2)} disabled={sending}>← Back</GhostBtn>
              <PrimaryBtn onClick={sendLetter} disabled={!canSend3 || sending} bg={P.terracotta}>
        ...
        {sent && (
          <div style={{ textAlign:"center", padding:"48px 0", animation:"fadeUp 0.5s ease-out" }}>
            ...
            <p style={{ color:P.driftwood, fontFamily:BODY, fontSize:13, letterSpacing:1, marginBottom:32 }}>
              Your letter was sent to {recipient.name} at {recipient.email}
            </p>
              ...
              <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"center", gap:2, paddingBottom:85 }}>
                {placed.slice(0,8).map((f,i) => (
                  <div key={i} style={{
                    ...
                    <StemmedFlower flower={f} size={0.9} stemCurve={f.stemCurve||0} stemBend={f.stemBend??0} />
                  </div>
                ))}
              </div>
              <BouquetPaper />
